...
function needsEventSelection(selfPlayer, enemyPlayer, eventCard, side) {
    switch (eventCard.name) {
        ...
        case '創作料理':
            return selfPlayer.hand.length >= 2;
        case '食材探索':
            return true;
        case '緊急調理':
            return selfPlayer.score <= 5 && selfPlayer.hand.length >= 1;
        default:
            return false;
    }
}
...
function startEventSelection(selfPlayer, enemyPlayer, eventCard, side) {
    ...
    switch (eventCard.name) {
        ...
        case '創作料理': {
            ...
            context.description = '捨てる材料を2枚選んでください。3点獲得します。';
            break;
        }
        ...
        case '緊急調理': {
            ...
            context.description = '捨てる材料を1枚選んでください。3点獲得します。';
            break;
        }
...
function executeEventEffect(selfPlayer, enemyPlayer, eventCard, side, extra) {
    ...
    switch (eventCard.name) {
        ...
        case '創作料理': {
            const ids = selectedIds.length === 2 ? selectedIds : selfPlayer.hand.slice(0, 2).map(card => card.id);
            ...
            ids.forEach(id => {
                const card = removeCardByIdFromArray(selfPlayer.hand, id);
                if (card) moveCardToDiscard(card);
            });

            selfPlayer.score += 3;
            selfPlayer.lockedCookingThisTurn = true;
            addLog(`${actorName}は創作料理で3点獲得しました。今ターン他の料理は作れません。`);
            break;
        }
        ...
        case '大掃除': {
            const count = discardAllIngredientHand(enemyPlayer);
            for (let i = 0; i < count; i++) {
                drawOneResolved(enemyPlayer);
            }
            addLog(`${actorName}は大掃除で相手の手札を引き直しさせました。`);
            break;
        }
        ...
        case '緊急調理': {
            ...
            const id = selectedIds[0] || selfPlayer.hand[0]?.id;
            if (!id) {
            ...
            selfPlayer.score += 3;
            selfPlayer.lockedCookingThisTurn = true;
            addLog(`${actorName}は緊急調理で3点獲得しました。今ターン他の料理は作れません。`);
            break;
        }
...
function playerEndTurn() {
    if (GameState.gameEnded) return;
    if (GameState.currentTurn !== 'player') return;
    if (GameState.selectionMode === 'event-target' || GameState.selectionMode === 'set-confirm' || GameState.selectionMode === 'event-confirm' || GameState.selectionMode === 'set-view') return;
